...
namespace Utils
{
	namespace String
	{
        ...
        // Project s(UTF-8, may contain CJK) to an ASCII "pinyin/string" and
        // then run a case-insensitive substring search against 'what'
        bool containsIgnoreCasePinyin(const std::string& s, const std::string& what)
        {
            if (s.empty() || what.empty()) return false;

            // Query (ASCII) to lowercase
            std::string q = toLowerAscii(what);

            // 快速路径：如果字符串全是ASCII字符，直接转换并匹配
            bool allAscii = true;
            for (unsigned char c : s) { if (c & 0x80) { allAscii = false; break; } }
            if (allAscii)
                return toLower(s).find(q) != std::string::npos;

            // 用于逐个字匹配拼音首字母
            std::string projection;
            projection.reserve(s.size() * 3);

            size_t cursor = 0;
            size_t qCursor = 0;  // 用于遍历查询拼音的游标
			bool started = false; // 是否已经消费过查询字符

            while (cursor < s.size() && qCursor < q.size())
            {
				// 先吃掉查询侧的连续分隔符（保持“可折叠”）
				while (qCursor < q.size() && isAsciiDelimiter((unsigned char)q[qCursor])) {
					qCursor++;
				}
				if (qCursor >= q.size()) break;
		
                unsigned char c0 = (unsigned char)s[cursor];

                if ((c0 & 0x80) == 0)
                {
		            // ASCII 分支：允许分隔符可跳过/可折叠；非分隔符按原“逐字符”逻辑
		            unsigned char ls = toLowerAsciiChar(c0);
		
		            if (isAsciiDelimiter(ls)) {
		                // 标题侧分隔符：跳过它（查询侧分隔符上面已吃掉）
		                cursor++;
		                continue;
		            }
		
		            unsigned char lw = (unsigned char)q[qCursor];
		            // q 已 toLowerAscii，这里直接对比
		            if (ls == lw) {
		                projection.push_back((char)ls);
		                qCursor++;
		                cursor++;
						started = true;
		                continue;
		            } else {
		                // 新：如果还没开始匹配，允许“滑动起点”——跳过标题这个 ASCII 字符
		                if (!started) {
		                    cursor++;
		                    continue;
		                }
                ...
                unsigned int cp = chars2Unicode(s, cursor); // advances cursor
                if (cursor == before) { cursor++; continue; } // safety

                if (cp < 0x80)
                {
                    projection.push_back((char)std::tolower((int)cp));
                    continue;
                }

                // 对多音字的拼音进行逐字匹配
                if (const char* py = codepointToPinyin((uint32_t)cp))
                {
                    // 尝试使用每个拼音首字母与查询中的拼音匹配
                    bool matched = false;
                    for (const char* p = py; *p; ++p)
                    {
                        if (toLowerAsciiChar((unsigned char)*p) == (unsigned char)q[qCursor])
                        {
                            matched = true;
                            qCursor++;  // 移动查询拼音的游标
							started = true;
                    ...
                    if (matched)
                    {
                        projection.push_back(std::tolower((int)(unsigned char)py[0]));
                    }
                    else
                    {
                        // 如果没有匹配，跳过当前字
